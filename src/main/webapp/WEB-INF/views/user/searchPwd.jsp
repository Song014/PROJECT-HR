			...
			<div class="signup">
				<div class="column">
					<input type="button" value="비밀번호 찾기" class="searchpwd"
						onclick="location.href='searchPwd.jsp'">
					<input type="button" value="로그인페이지로 돌아가기" onclick="location.href='login.jsp'">
				</div>
			</div>
		</div>
		...
		function sucFuncJson(data) {
			...
			if (data != "없어용") {
				...
				/* if(confirm("로그인 페이지로 돌아가시겠습니까?")){
					window.location.href = "/WEB-INF/views/user/login.jsp";
				} else {
					window.location.href = "/WEB-INF/views/user/searchPwd.jsp";
				} */
			} else {
				alert("아이디 또는 주민등록번호를 잘못 입력하셨습니다. 다시 입력하세요.");
				window.location.href = "searchPwd.jsp";
			}
		}
